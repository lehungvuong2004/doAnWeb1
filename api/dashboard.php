...
<head>
    ...
    <style>
        ...
        .header{background:linear-gradient(135deg,#667eea,#764ba2);color:#fff;padding:1rem 2rem}
        .container-main{display:flex}
        .sidebar{width:30%;background:#fff;padding:2rem 0;border-right:1px solid #e0e0e0}
        .content{width:70%;padding:2rem}
        .menu-item{padding:0.75rem 2rem;cursor:pointer;transition:0.3s}
...
<div class="header d-flex justify-content-between align-items-center">
    ...
    <div class="d-flex align-items-center gap-3">
        ...
        <div class="admin-profile d-flex align-items-center gap-2"><img src="https://via.placeholder.com/32" style="border-radius:50%"> Admin</div>
    </div>
</div>
...
<div class="container-main">
    <div class="sidebar">
        ...
        <?php foreach ($allowedTables as $tb): ?>
            <div class="menu-item <?= $table === $tb ? 'active' : '' ?>" onclick="location.href='?table=<?= $tb ?>'">
                <?php echo ucfirst($tb); ?>
            </div>
        <?php endforeach; ?>
    </div>
            ...
            <table class="table mb-0">
                ...
                <?php foreach ($currentData as $row): ?>
                    <tr>
                        ...
                        <?php endforeach; ?>
                        <td>
                            <button class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#editModal" onclick='openEdit(<?= json_encode($row) ?>)'><i class="bi bi-pencil"></i> Sửa</button>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form id="addForm" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-header"><h5 class="modal-title">Thêm <?= ucfirst($table) ?></h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form id="editForm" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-header"><h5 class="modal-title">Sửa <?= ucfirst($table) ?></h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                <div class="modal-body" id="editFormBody"></div>
                <div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Đóng</button><button type="submit" class="btn btn-primary">Cập Nhật</button></div>
            </form>
        </div>
    </div>
</div>
...
<script>
    ...
    function makeFieldHTML(name, label, value = '') {
        ...
        let type = 'text';
        if (name === 'giaMonAn' || name === 'giaGiam' || name === 'tienLuong' || name === 'soNguoi') type = 'number';
